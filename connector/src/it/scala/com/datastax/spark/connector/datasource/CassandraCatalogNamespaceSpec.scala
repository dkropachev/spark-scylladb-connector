...
class CassandraCatalogNamespaceSpec extends CassandraCatalogSpecBase {
  ...
  it should "list a keyspaces in a keyspace"  in {
    spark.sql(s"CREATE DATABASE IF NOT EXISTS $defaultKs WITH DBPROPERTIES (${networkTopologyDbProperties(5)})")
  ...
  it should "be able to create a new keyspace" in {
    ...
    spark.sql(s"CREATE DATABASE $defaultKs WITH DBPROPERTIES (${networkTopologyDbProperties(5)})")
    waitForKeyspaceToExist(defaultKs)
  ...
  it should "explain Cassandra specific properties in the describe statement" in {
    ...
    properties should include(s"$defaultDatacenter,5")
  ...
  it should "alter a keyspace" in {
    ...
    spark.sql(s"CREATE DATABASE IF NOT EXISTS $defaultKs WITH DBPROPERTIES (${networkTopologyDbProperties(5)})")
    waitForKeyspaceToExist(defaultKs)
    spark.sql(s"ALTER NAMESPACE $defaultKs SET DBPROPERTIES ($defaultDatacenter='2')")
    eventually {
      getMetadata().getKeyspace(defaultKs).get().getReplication.get(defaultDatacenter) shouldBe ("2")
    }
  }
